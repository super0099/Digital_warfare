...
public class WarfareServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(!Tools.isNotNull(method))method="index";
        try {
            Method doMothod =this.getClass().getMethod(method,HttpServletRequest.class,HttpServletResponse.class);
            doMothod.invoke(this,request,response);
            System.out.println("调用了"+getClass().getSimpleName()+"的方法["+method+"]");
        } catch (NoSuchMethodException | IllegalAccessException | InvocationTargetException e) {
            e.printStackTrace();
            String errorMsg = getClass().getSimpleName()+"的方法["+method+"]不存在,或者无法调用";
